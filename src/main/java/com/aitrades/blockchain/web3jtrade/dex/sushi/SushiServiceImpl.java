package com.aitrades.blockchain.web3jtrade.dex.sushi;
...
public class SushiServiceImpl implements EthereumDexContractService {
	...
	public BigInteger getAmountsIn(Credentials credentials, BigDecimal inputEthers, BigDecimal slipage,
		...
		EthereumDexContract sushiContract = new EthereumDexContract(TradeConstants.ROUTER_MAP.get(TradeConstants.SUSHI),
																	...
																	credentials, 
																	customGasProvider);
		BigInteger amountsOut = (BigInteger) sushiContract.getAmountsIn(Convert.toWei(inputEthers, Convert.Unit.ETHER).toBigInteger(), memoryPathAddress)
														  .flowable()
														  .blockingSingle()
														  .parallelStream()
														  .findFirst()
														  .get();
		double calculaterAmountOutsWithSlipage  = amountsOut.doubleValue() * slipage.doubleValue();
		return new BigDecimal(amountsOut.doubleValue() - calculaterAmountOutsWithSlipage).setScale(0, RoundingMode.DOWN).toBigInteger();
	}
	...
	public BigInteger getAmountsOut(Credentials credentials, BigDecimal inputTokens, BigDecimal slipage,
		...
		BigInteger amountsIn = (BigInteger) sushiContract.getAmountsOut(Convert.toWei(inputTokens, Convert.Unit.ETHER).toBigInteger(), memoryPathAddress)
														 .flowable()
														 .blockingSingle()
														 .stream()
