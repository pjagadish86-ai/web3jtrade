...
public class LiquidityEventOrReserversFinder {
	...
	public boolean hasReservesMetWithInputAmount(final String route, final String  pairAddress, final Credentials credentials,final  BigInteger gasPrice, final  BigInteger gasLimit, BigInteger inputAmount, final String  gasMode) throws Exception {
		...
		return reserves != null && reserves.component1().compareTo(BigInteger.ZERO) > ZERO && reserves.component2().compareTo(BigInteger.ZERO) >= ZERO;
	}
	
	@SuppressWarnings({ "rawtypes", "unchecked" })
	public List<Boolean> hasLiquidityOrReservesV2(final String route, 
												  final String  pairAddress, 
												  final Credentials credentials,
												  final BigInteger gasPrice, 
												  final BigInteger gasLimit, 
												  final BigInteger inputAmount, 
												  final String  gasMode) throws Exception {
		List<Boolean> results = new ArrayList(1);
//		Flowable.zip(new EthereumDexContract(pairAddress, web3jServiceClientFactory.getWeb3jMap(route).getWeb3j(), credentials).getReserves().flowable(), 
//												  EthereumDexEventHandler.mintEventFlowables(web3jServiceClientFactory.getWeb3jMap(route).getWeb3j(), pairAddress, TradeConstants.ROUTER_MAP.get(pairAddress)),
...
//											 .blockingSubscribe(results :: add);
	...
	private EthLog hasAddedLiquidityEvent(final String  route, final String  pairAddresss) throws Exception{
		try {
//			return EthereumDexEventHandler.mintEventFlowables(web3jServiceClientFactory.getWeb3jMap(route).getWeb3j(), 
//															  pairAddresss, 
